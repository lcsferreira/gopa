$(document).ready(function (e) {
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data == "invalid") {
          ...
          setTimeout(function () {
            window.location.href = "cardUploadTranslated.php?id=" + data;
          }, 2000);
        }
      },
      error: function (e) {
  ...
  $("#form-contact").on("submit", function (e) {
    ...
    $("#msg").html("Uploading...").fadeIn();
    $.ajax({
      url: "cardUploadContactTranslated.php",
      type: "POST",
      data: new FormData(this),
      contentType: false,
      cache: false,
...
function getPDF(id) {
  pdfjsLib
    .getDocument(
      "https://work.globalphysicalactivityobservatory.com/uploads/card_translated/" +
        id +
        ".pdf"
    )
...
function saveComment() {
  ...
  let request = $.ajax({
    method: "POST",
    url: "../../ajaxQuerys/cards/cardCommentsContactsTranslated.php",
...
function saveValue(status, id) {
  let request = $.ajax({
    method: "POST",
    url: "../../ajaxQuerys/cards/saveStatusTranslated.php",
  ...
  request.done(function (msg) {
    ...
    if (msg == "approved") {
      window.location.href =
        "../../pages/cardUpload/cardApproved.php?id=" +
        id +
        "&version=translated";
    } else {
      window.location.href =
...
function submitValueAdmin() {
  ...
  if (status == "review") {
    let request = $.ajax({
      method: "POST",
      url: "../../ajaxQuerys/cards/sendToContactReviewTr.php",
      data: { id: id },
      dataType: "text",
      type: "post",
      contentType: "application/x-www-form-urlencoded",
